...
* [`src/clj/clj_physics/`](src/clj/clj_physics/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/clj_physics/`](src/cljs/clj_physics/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/clj_physics/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `clj-physics` project root directory
...
in the [`src/clj/clj_physics/`](src/clj/clj_physics/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/clj_physics/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns clj-physics.example
  (:require [clj-physics.config :as config])
